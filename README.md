...
| `01_statistical_analysis.R` | R script: hierarchical log-linear model, Pearson chi-squared tests, synchrony index with paired Wilcoxon signed-rank test, and G² (likelihood-ratio) tests for within-instar temporal shift |
| `02_gating_analysis.R` | R script: within-instar daily ecdysis distribution summary (descriptive) |
...
No additional packages beyond `MASS` are required; all other statistical procedures (paired Wilcoxon signed-rank test, G² calculations) are implemented in base R or directly within the analysis script.
...
| Date | Changes |
|---|---|
| 2026-05-23 | `01_statistical_analysis.R` revised. Section 4: paired Wilcoxon signed-rank test now computed without continuity correction; the test statistic *z* is derived analytically from V₊, E(V), and SD(V) with tie correction, ensuring mutual consistency with the reported two-sided *p* value. Section 5: switched from Pearson chi-squared to G² (likelihood-ratio) test for all 2 × 3 (day × observation time) contingency tables to provide a consistent treatment across all instars; the G² statistic remains well defined when some observed cells contain zero counts. |
